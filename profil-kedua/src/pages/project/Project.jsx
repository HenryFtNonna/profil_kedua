        ...
        <AnimatePresence>
          {canScrollLeft && (
            <motion.div
              ...
              exit={{ opacity: 0, x: -10 }}
              className="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 z-30 w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#111]/80 backdrop-blur-md border border-gray-700 flex items-center justify-center shadow-lg text-white group cursor-grab hover:border-blue-500 transition-colors"
            >
              <Tooltip text="Swipe it!" />
              <ArrowLeftIcon />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {canScrollRight && (
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: 10 }}
              className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 z-30 w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#111]/80 backdrop-blur-md border border-gray-700 flex items-center justify-center shadow-lg text-white group cursor-grab hover:border-blue-500 transition-colors"
            >
              <Tooltip text="Swipe it!" />
              <ArrowRightIcon />
            </motion.div>
...
const Tooltip = ({ text }) => (
  <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 group-hover:-translate-y-1 transition-all duration-300 pointer-events-none z-40">
    <div className="relative bg-[#1a1a1a] border border-blue-500/50 text-blue-400 text-[10px] font-mono tracking-widest uppercase px-3 py-1.5 rounded-lg shadow-blue-500/30 whitespace-nowrap">
      {text}
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-3 mt-auto pt-2 border-t border-gray-800/50">
          {project.github && (
            <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex-1 flex justify-center items-center gap-2 py-2.5 rounded-lg border border-gray-700 text-gray-300 hover:text-white hover:border-blue-500 hover:bg-blue-500/10 transition-all font-semibold text-sm">
              Code
            </a>
          )}
          {project.live && (
            <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex-1 py-2.5 rounded-lg bg-white hover:bg-gray-200 text-black text-center transition-all font-bold text-sm shadow-md">
              Live Demo
            </a>
          )}
        </div>
